...
$queryDataService = mysqli_query($connection, "SELECT * FROM type_of_service WHERE deleted_at=1 ORDER BY id DESC");
?>

<div class="wrapper">
    <div class="card mt-3">
        <div class="card-body">
            <h3 class="card-title">Recycle Bin Data Service</h3>
            <div align="right" class="button-action">
                <a onclick="return confirm ('Apakah anda yakin akan memulihkan semua data?')"
                    href="?pg=restore-data-service&restore-all=process">
                ...
                <a onclick="return confirm ('Apakah anda yakin akan menghapus semua data?')"
                    href="?pg=restore-data-service&delete-all=process">
                ...
                <tbody>
                    ...
                    while ($rowDataService = mysqli_fetch_assoc($queryDataService)) : ?>
                        <tr>
                            ...
                            <td>
                                <a href="?pg=restore-data-service&restore=<?php echo $rowDataService['id'] ?>">
                                ...
                                <a onclick="return confirm ('Apakah anda yakin akan memulihkan data ini?')"
                                    href="?pg=restore-data-service&restore-row=<?php echo $rowDataService['id'] ?>">
                                    <button class="btn btn-light">
                                ...
                                <a onclick="return confirm ('Apakah anda yakin akan menghapus data ini?')"
                                    href="?pg=restore-data-service&delete-row=<?php echo $rowDataService['id'] ?>">
                                    <button class="btn btn-light">
